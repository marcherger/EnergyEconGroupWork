...
class MAC:
	def __init__(self, α=0.5, γ=1, pe=1, ϕ=0.25, γd=100): 
        # Initialize the 'db' attribute as a dictionary
		self.db = {}
        
        # Add values for all relevant parameters to the 'db' dictionary
		self.db['α'] = α
		self.db['γ'] = γ
		self.db['pe'] = pe
		self.db['ϕ'] = ϕ
	...
	def C(self, E):
		F_E = self.db['γ'] * E**self.db['α']

		return ( F_E - self.db['pe'] * E )
	...
	def Ctilde(self, E):
		F_E = self.db['γ'] * E**self.db['α']
		M = self.db['ϕ'] * E
		D_M = (self.db['γd'] * (self.db['ϕ'] * E)**2) / 2

		return ( (F_E) - self.db['pe'] * E - D_M)
